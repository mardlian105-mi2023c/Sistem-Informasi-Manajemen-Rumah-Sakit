...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-xl shadow-md overflow-hidden p-6 max-w-3xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Data Rawat Inap</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select name="pasien_id" id="pasien_id" required
                        ...
                        @foreach($pasien as $pasien)
                            <option value="{{ $pasien->id }}" 
                                {{ old('pasien_id', $rawatinap->pasien_id) == $pasien->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="kamar" id="kamar" 
                        value="{{ old('kamar', $rawatinap->kamar) }}" required
                        class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                ...
                <div>
                    ...
                    <div class="flex space-x-4">
                        @foreach (['dirawat', 'selesai', 'pindah'] as $status)
                            <label class="inline-flex items-center">
                                <input type="radio" name="status" value="{{ $status }}"
                                    class="text-blue-600 focus:ring-blue-500"
                                    {{ old('status', $rawatinap->status) == $status ? 'checked' : '' }}>
